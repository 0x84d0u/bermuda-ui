...
import * as UI from "./Drawer.ui";
import * as Types from './Drawer.types'
import { Button, Typography } from "@bermuda-ui/foundation";
            ...
            >
                <UI.Header>
                    <div className="flex-1">
                        {title && <Typography kind="heading" size={2} id={titleId}>{title}</Typography>}
                        {headerContent}
                    </div>
                    <Button {...closeProps} onClick={close} />
                </UI.Header>
                <UI.Body id={descriptionId}>{children}</UI.Body>
                {footerContent && <UI.Footer>{footerContent}</UI.Footer>}
            </UI.Root>
        </>
    );
};